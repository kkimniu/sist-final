...
import io.cavia.trader.module.notice.excetion.InvalidNoticeContentException;
import io.cavia.trader.module.notice.excetion.InvalidNoticeTitleException;
import io.cavia.trader.module.notice.excetion.NoticeSaveFailedException;
 ...
 * isBlank를 사용한이유는 isEmpty보다 ""되어있는것도 공백으로 ture 한다고함 그래서 사용
 * noticeDto.setCreate_at(new Timestamp(System.currentTimeMillis())); 이유는
 * 컴럼값이 Timestamp로 되어잇어서 거기에 맞추고 싶어서 사용함
...
public class NoticeController {
    ...
    public ResponseEntity<String> saveNotice(@RequestBody NoticeDto noticeDto){
        if(noticeDto.getTitle() ==null||noticeDto.getTitle().isBlank()) {
            throw new InvalidNoticeTitleException("제목은 비워둘 수 없습니다");
        }
        if(noticeDto.getContent() ==null||noticeDto.getContent().isBlank()){
            throw new InvalidNoticeContentException("내용은 비워둘 수 없습니다");
        }
        noticeDto.setCreated_at(new Timestamp(System.currentTimeMillis()));
